...
现状评估、目标资产模型、功能需求与验收标准见 [产品规划 PRD](docs/PRODUCT_PRD.md)；建议交付顺序与待办清单见 [迭代路线](docs/PRODUCT_ROADMAP.md)。当前已实施经确认分期计划的第一期“基础可信性修正”；范围、验收步骤、备份恢复记录及后续边界见 [第一期交付说明](docs/PHASE1_DELIVERY.md)。后续各期仍需分别验收后启动。
...
- 预览版用于内部验收和测试；“当前发布原型”是可切换的原型版本指针。版本材料仍是可继续整理的工作资料，尚不构成不可变发布快照。
- 只有未被预览版或发布版指针引用的版本才能删除，删除时同步清理原始包和预览文件。
- 上传版本时可选填受限富文本格式的“版本变更内容”，用于记录相较所选比较基线的变化；时间线可通过整行或“查看变更”在右侧抽屉阅读，并支持后续修订。
...
- 每个原型使用持久化计数分配版本号；删除最高版本后继续递增，并发上传不会分配重复编号。允许编号出现缺口。
- 上传时可选择同一原型的成功版本作为比较基线，默认最近成功版本，也可明确选“无”。旧版本不补造基线，显示“未记录”；删除基线版本后保留其编号。
- 版本时间线和工作空间支持下载 HTML/ZIP 原件，需登录并校验所有权；预览处理中或失败也可以下载已保存的原件。
- 搜索区分资产信息与版本内容命中；版本结果直接进入命中的历史工作空间。搜索错误单独提示，不显示为“没有结果”。
...
首次结构迁移前已完成停写的一致性备份，并在独立 PostgreSQL / MinIO 环境恢复核验。具体位置、结果和回退原则见 [第一期交付说明](docs/PHASE1_DELIVERY.md#备份迁移与回退)。日常自动备份和恢复工具留待第二期固化。

下列命令只备份数据库，完整备份还需在同一停写窗口保存对象存储：
